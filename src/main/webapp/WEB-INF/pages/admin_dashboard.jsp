...
<body>
    <div class="dashboard-wrapper">
        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="#" class="nav-item" onclick="switchTab('taskRequests', this)"><i data-lucide="git-pull-request"></i> Task Requests <c:if test="${requestedCount > 0}"><span class="badge badge-requested" style="font-size:0.6rem; padding:2px 6px; margin-left:4px;">${requestedCount}</span></c:if></a>
                <a href="#" class="nav-item" onclick="switchTab('verifyCompletions', this)"><i data-lucide="check-square"></i> Verify <c:if test="${completedCount > 0}"><span class="badge badge-completed" style="font-size:0.6rem; padding:2px 6px; margin-left:4px;">${completedCount}</span></c:if></a>
                <a href="#" class="nav-item" onclick="switchTab('users', this)"><i data-lucide="users"></i> Users</a>
                <a href="#" class="nav-item" onclick="switchTab('applications', this)"><i data-lucide="user-plus"></i> Applications <c:if test="${pendingApps > 0}"><span class="badge badge-pending" style="font-size:0.6rem; padding:2px 6px; margin-left:4px;">${pendingApps}</span></c:if></a>
                <a href="#" class="nav-item" onclick="switchTab('supportMessages', this)"><i data-lucide="mail"></i> Support <c:if test="${supportCount > 0}"><span class="badge badge-blue" style="font-size:0.6rem; padding:2px 6px; margin-left:4px;">${supportCount}</span></c:if></a>
            ...
            <div class="sidebar-footer">
                ...
                <a href="#" class="footer-nav-item" onclick="switchTab('supportMessages', null)"><i data-lucide="help-circle"></i> Support</a>
        ...
        <div class="main-content-wrapper">
            ...
            <div class="top-bar">
                <div class="top-bar-search">
                    ...
                    <input type="text" id="adminSearchInput" placeholder="Search reports, animals..." onkeyup="filterAdminReports()">
                </div>
            ...
            <main class="main-content admin-content">
                ...
                <div id="tab-reports" class="tab-content active">
                    ...
                    <div class="search-filter-bar" style="margin-bottom: 14px;">
                        <input type="text" id="adminSearchInline" placeholder="Search reports..." onkeyup="filterAdminReports()">
                            ...
                            <div class="report-card cat-${report.category == 'Carcass' ? 'carcass' : report.category == 'Injured' ? 'injured' : 'wild'}" data-category="${report.category}" data-status="${report.status}" data-search="${report.animalType} ${report.description} ${report.locationDesc} ${report.reporterName}">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="card-meta">
                                        ...
                                        <span class="badge badge-${report.status == 'PENDING' ? 'pending' : report.status == 'REQUESTED' ? 'requested' : report.status == 'ASSIGNED' ? 'assigned' : report.status == 'COMPLETED' ? 'completed' : 'resolved'}">${report.status}</span>
                                    </div>
                                    ...
                                    <form action="${pageContext.request.contextPath}/admin/report/action" method="POST" class="card-actions" style="margin: 0 0 6px 0;">
                                        <input type="hidden" name="action" value="updateStatus">
                                        <input type="hidden" name="reportId" value="${report.id}">
                                        <select name="status" class="status-select">
                                            <option value="PENDING" ${report.status == 'PENDING' ? 'selected' : ''}>Pending</option>
                                            <option value="REQUESTED" ${report.status == 'REQUESTED' ? 'selected' : ''}>Requested</option>
                                    ...
                                    <div class="card-actions" style="margin:0 0 6px 0; gap:4px;">
                                        <select id="forceStaff-${report.id}" class="status-select" style="flex:1;">
                                            <option value="">Select Staff</option>
                                            <c:forEach items="${staffUsers}" var="s"><option value="${s.id}">${s.name}</option></c:forEach>
                                        </select>
                                        <button type="button" class="btn btn-approve btn-sm" style="margin:0;" onclick="adminForceAssign('${pageContext.request.contextPath}',${report.id})"><i data-lucide="user-plus" style="width:13px;height:13px;"></i> Assign</button>
                                    </div>
                                    </c:if>

                                    <!-- Archive/Delete -->
                                    <form action="${pageContext.request.contextPath}/admin/report/action" method="POST" style="margin: 0;" onsubmit="return confirm('Delete this report?');">
                ...
                <div id="tab-taskRequests" class="tab-content">
                    <div class="section-header"><i data-lucide="git-pull-request"></i><h2>Task Assignment Requests</h2></div>
                    <p class="section-desc">Staff have requested to be assigned to these incidents. Approve or reject.</p>
                    ...
                    <c:forEach items="${taskRequests}" var="report">
                        <div class="task-request-card">
                            <div class="app-info">
                                ...
                                <div class="app-meta">
                                    <c:if test="${report.locationDesc != null}"><i data-lucide="map-pin" style="width:12px;height:12px;display:inline;"></i> ${report.locationDesc} &middot; </c:if>
                            ...
                            <div class="card-actions" id="taskReq-actions-${report.id}">
                                <button type="button" class="btn btn-approve btn-sm" style="margin:0;" onclick="adminTaskAction('${pageContext.request.contextPath}',${report.id},'approve',this)"><i data-lucide="check" style="width:13px;height:13px;"></i> Approve</button>
                                <button type="button" class="btn btn-reject btn-sm" style="margin:0;" onclick="adminTaskAction('${pageContext.request.contextPath}',${report.id},'reject',this)"><i data-lucide="x" style="width:13px;height:13px;"></i> Reject</button>
                            </div>
                        </div>
                    </c:forEach>
                    </div>
                ...
                <div id="tab-verifyCompletions" class="tab-content">
                    <div class="section-header"><i data-lucide="check-square"></i><h2>Verify Completion Reports</h2></div>
                    <p class="section-desc">Staff have submitted completion reports. Verify or send back.</p>
                    <div style="margin-top: 14px;">
                    <c:forEach items="${completionReports}" var="report">
                        <div class="task-request-card" style="border-left-color: var(--color-info, #1976d2);">
                            <div class="app-info">
                                <div class="app-name">${report.animalType} — ${report.category}</div>
                                <div class="app-meta">
                                    <c:if test="${report.locationDesc != null}"><i data-lucide="map-pin" style="width:12px;height:12px;display:inline;"></i> ${report.locationDesc} &middot; </c:if>
                            ...
                            <div class="card-actions" id="compReq-actions-${report.id}">
                                <button type="button" class="btn btn-approve btn-sm" style="margin:0;" onclick="adminTaskAction('${pageContext.request.contextPath}',${report.id},'verify',this)"><i data-lucide="check-circle" style="width:13px;height:13px;"></i> Verify</button>
                                <button type="button" class="btn btn-reject btn-sm" style="margin:0;" onclick="adminTaskAction('${pageContext.request.contextPath}',${report.id},'rejectCompletion',this)"><i data-lucide="rotate-ccw" style="width:13px;height:13px;"></i> Send Back</button>
                            </div>
                        </div>
                    </c:forEach>
                    </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <c:forEach items="${allUsers}" var="user">
                                <tr>
                                    <td style="font-family: monospace; color: var(--color-text-muted);">#${user.id}</td>
                                    <td>
                                        <div style="display:flex; align-items:center; gap:8px;">
                                            <c:if test="${user.profileImage != null && !user.profileImage.isEmpty()}">
                                                <img src="${pageContext.request.contextPath}/${user.profileImage}" 
                                                     onerror="this.style.display='none';" 
                                                     style="width:28px; height:28px; border-radius:50%; object-fit:cover;" alt="User Avatar">
                                            </c:if>
                                    ...
                                    <td>
                                        <div class="card-actions" style="display:flex; gap:8px;">
                                            <form action="${pageContext.request.contextPath}/admin/user/action" method="POST" style="margin:0; display:flex; gap:4px; align-items:center;">
                                                ...
                                                <button type="submit" class="btn btn-primary btn-sm" style="margin:0;">Save</button>
                                            </form>
                                            <form action="${pageContext.request.contextPath}/admin/user/action" method="POST" style="margin:0;" onsubmit="return confirm('Delete this user?');">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="userId" value="${user.id}">
                                                <input type="hidden" name="reason" value="Admin manual deletion">
                                                <button type="submit" class="btn btn-danger btn-sm" style="margin:0;"><i data-lucide="trash-2" style="width:14px;height:14px;"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <c:forEach items="${applications}" var="app">
                        <div class="app-card">
                            <div class="app-info">
                                ...
                                <div class="app-meta">${app.userEmail} &middot; Role: <strong>${app.requestedRole}</strong> &middot; <span class="badge badge-${app.status == 'PENDING' ? 'pending' : app.status == 'APPROVED' ? 'green' : 'red'}">${app.status}</span></div>
                            </div>
                            <c:if test="${app.status == 'PENDING'}">
                                <div class="app-actions">
                                    <form action="${pageContext.request.contextPath}/admin/application/action" method="POST" style="margin:0;">
                                        ...
                                        <button type="submit" class="btn btn-approve btn-sm" style="margin:0;"><i data-lucide="check" style="width:13px;height:13px;"></i> Approve</button>
                                    </form>
                                    <form action="${pageContext.request.contextPath}/admin/application/action" method="POST" style="margin:0;">
                                        <input type="hidden" name="appId" value="${app.id}">
                    ...
                    <c:forEach items="${deletionHistory}" var="item">
                        <div class="history-card" style="display:flex; justify-content:space-between; align-items:center;">
                            <div style="display:flex; gap:12px; width:100%;">
                                <div class="history-icon"><i data-lucide="${item.entityType == 'REPORT' ? 'file-x' : item.entityType == 'SUPPORT' ? 'mail-x' : 'user-x'}"></i></div>
                                <div class="history-body" style="flex:1;">
                                    <div class="history-title">${item.entityType} #${item.entityId} — Deleted</div>
                                    <div class="history-meta">By: ${item.deletedBy} &middot; <fmt:formatDate value="${item.deletedAt}" pattern="MMM dd, HH:mm" /></div>
                                    <details style="margin-top: 6px;">
                                        <summary style="font-size: 0.78rem; color: var(--color-primary); cursor: pointer;">View Archive</summary>
                                        <pre style="background: var(--color-bg-light); padding: 10px; font-size: 0.78rem;">${item.entityData}</pre>
                                    </details>
                                </div>
                                <div style="display:flex; align-items:center;">
                                    <form action="${pageContext.request.contextPath}/admin/restore/action" method="POST" style="margin:0;" onsubmit="return confirm('Restore this record?');">
                                        <input type="hidden" name="historyId" value="${item.id}">
                                        <button type="submit" class="btn btn-primary btn-sm" style="margin:0;"><i data-lucide="rotate-ccw" style="width:14px;height:14px;"></i> Restore</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <div id="tab-supportMessages" class="tab-content">
                    <div class="section-header"><i data-lucide="mail"></i><h2>Support Messages</h2></div>
                    <div style="margin-top:14px;">
                    <c:forEach items="${supportMessages}" var="msg">
                        <div class="support-msg-card" style="display:flex; gap:12px; align-items:flex-start;">
                            ...
                            <div class="support-msg-body" style="flex:1;">
                                ...
                                <div class="support-msg-meta">${msg.userName} &middot; ${msg.userEmail}</div>
                                <div class="support-msg-text">${msg.message}</div>
                            </div>
                            <div class="support-msg-actions">
                                <form action="${pageContext.request.contextPath}/admin/support/action" method="POST" style="margin:0;" onsubmit="return confirm('Delete this support message?');">
                                    <input type="hidden" name="action" value="delete">
    ...
    <div id="settingsModal" class="modal-backdrop hidden">
        <div class="modal-content" style="max-width:420px;">
            ...
            <p style="color:var(--color-text-muted);font-size:0.88rem;margin-bottom:16px;">Customize your console experience.</p>
            <div class="settings-section">
                <div class="settings-section-title">Appearance</div>
                <div class="theme-toggle-row">
                    <span class="theme-toggle-label"><i data-lucide="sun"></i> Light Mode</span>
                    <label class="toggle-switch"><input type="checkbox" id="themeToggleCheckbox" onchange="toggleTheme()"><span class="toggle-slider"></span></label>
                </div>
            </div>
            <div class="modal-actions" style="margin-top:20px;"><button type="button" onclick="closeSettingsModal()" class="modal-submit-btn" style="flex:none;width:100%;">Done</button></div>
        </div>
    </div>
